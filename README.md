# 嘉得路网球对战平台（LTJIMMY 网球赛事）

微信小程序 + 微信云开发。当前阶段：**Figma 还原界面已落地，登录 / 赛事 / 报名 / 俱乐部 / 榜单 / 相册 / 店铺 / 参赛记录走云库；选手会员 L-ID 与 198 元年费规则已按四阶段接上**。微信支付、约战匹配本期不做。

运营后台在独立仓库 [jiadelu-admin](https://github.com/san31415926/jiadelu-admin)，和本仓库共用同一套云库。

> AI 协作者请先读 [`AGENTS.md`](AGENTS.md)，里面有项目硬约束和必读文档顺序；
> 具体开发规范在 `.cursor/rules/` 下（注释风格、小程序耦合约束、Figma 还原、资产流水线）。

## 仓库结构

| 目录 | 内容 |
| --- | --- |
| `miniprogram/` | 微信小程序工程（原生 + TypeScript） |
| `测试图/` | Figma 导出的定稿页面图与 3D 图标素材板，是视觉验收的基准 |
| `ui-slices/` | 早期切图脚本与产物（sharp 图像流水线） |
| `tools/` | 设计稿取色、图标包围盒检测、波浪路径提取、Figma 资产下载与压缩脚本 |
| `功能清单.md` | 旧版小程序功能盘点与新版归位表，开发范围以此为准 |

## 设计稿来源

Figma 文件「网球赛事总结」，file key `nzKWauHC37fyjpAtA3rjfd`，已定稿画板：

| 画板 | node id | 对应页面 |
| --- | --- | --- |
| Tennis Home / V5 默认 | `145:199` | 赛事首页（侧栏打开 / 各筛选是同一页状态） |
| Super Cup / V5 信息架构 | `230:158` | 超级杯 |
| 超级杯 / V5 / 俱乐部榜单 · 本月 | `287:292` | 俱乐部榜单 |
| My / Prototype Screen | `10:211` | 我的 |
| Player Rankings / Prototype Screen | `15:2` | 球员排行（TOP15 版） |
| Event Calendar / Prototype Screen | `17:92` | 赛事日历 |
| Gallery / Prototype Screen | `23:272` | 赛事相册 |
| Club / Prototype Screen | `23:343` | 俱乐部 |

画板宽 **750px**，与小程序 750rpx 基准一致，因此**设计稿 1px 直接写作 1rpx**。画板高 1184px
短于真机可视高度，页面按「头部固定高度 + 内容区流式」实现，不要按画板总高等比缩放。

设计 token 统一收在 `miniprogram/styles/tokens.wxss`，取值全部来自 Figma 节点，不是目视估值。

## 本地环境准备

### 小程序

改动小程序前建议先看 [`miniprogram/工程说明.md`](miniprogram/工程说明.md)，
里面有目录结构、`.json` 配置逐项说明（JSON 不支持注释，只能写在文档里），
以及「想改什么去动哪个文件」的速查表。代码文件本身每个都有详细注释。

1. 安装[微信开发者工具](https://developers.weixin.qq.com/miniprogram/dev/devtools/stable.html)稳定版。
2. 导入**仓库根目录**（不是 `miniprogram/`），编译类型选「小程序」。根目录的 `project.config.json` 会指向 `miniprogram/` 和 `cloudfunctions/`。
3. 把根目录 `project.config.json` 里的 `appid` 换成自己的 AppID。
4. TypeScript 由开发者工具的编译插件处理（`useCompilerPlugins: ["typescript"]`），无需额外构建步骤。
5. 云函数写好后，在工具里对 `cloudfunctions` 下每个文件夹右键「上传并部署：云端安装依赖」。

云开发环境 ID 填在 `miniprogram/config/env.ts`；各模块是否走假数据看同文件的 `USE_MOCK`。

### 图像处理脚本

`tools/` 与 `ui-slices/` 下的脚本依赖 `sharp`，当前复用了同级 `image-to-slice-tool` 项目的
`node_modules`（该目录不在本仓库内）。如需运行这些脚本，先在同级目录准备该项目：

```bash
git clone https://github.com/Asai2080/Figma-design.git image-to-slice-tool
cd image-to-slice-tool && npm install
```

脚本用途：

| 脚本 | 用途 |
| --- | --- |
| `tools/sample-design-colors.cjs` | 定点采样定稿图颜色，输出 hex |
| `tools/detect-icon-boxes.cjs` | 自动检测图标素材板上每个图标的包围盒 |
| `tools/extract-wave-path.cjs` | 提取头部渐变区下缘的波浪曲线，生成 SVG path |
| `tools/probe-crop.cjs` | 裁剪并放大指定区域，用于人工核对切图坐标 |
| `tools/build-mp-icons.cjs` | 把切图资产整理压缩进小程序 assets |
| `tools/fetch-figma-assets.ps1` | 下载 Figma MCP 导出的资产（地址 7 天过期） |
| `tools/build-figma-assets.cjs` | 压缩 Figma 原始资产到小程序显示尺寸 |
| `tools/build-event-detail-icons.cjs` | 压缩赛事详情八宫格到 128px |

## Figma → 代码工作流

通过 Figma 远程 MCP（`https://mcp.figma.com/mcp`）读取设计上下文，拿到每个节点的精确坐标、
颜色、字号、圆角与导出资产，再改写为 WXML/WXSS。要点：

- 简单几何图标（圆点、圆环、圆角条、椭圆光晕）用纯 WXSS 实现，不占包体积。
- 3D 图标与照片使用 Figma 导出的原始资产，按各自标注尺寸单独设定宽高，不套用统一尺寸。
- 底栏未选中图标在设计中带 `opacity: 52%`，用 CSS 控制，不要把它烘进图片。

小程序主包上限 2MB，`miniprogram/assets/` 当前约 220KB。

## 开发范围

本期实现 `功能清单.md` 中已有设计稿的页面；**对战 / 约战匹配不做**。底部三 Tab：赛事 / 超级杯 / 我的，
榜单作为首页宫格进入的二级页。

## 当前进度

| 页面 | Figma 节点 | 状态 |
| --- | --- | --- |
| 赛事首页 | `145:199` | 已按视觉刷新草稿 V5 还原（侧栏/筛选是同一页的状态） |
| 超级杯 | `230:158` | 已按视觉刷新草稿 V5 还原（Cover Flow，无侧栏） |
| 我的 | `302:103` | 已按视觉刷新草稿 V5 还原（更换背景 `401:359` 是同一页抽屉） |
| 球员排行 | `15:3` | 已还原 |
| 俱乐部榜单 | `287:292` | 已按视觉刷新草稿 V5 还原（本月/累计） |
| 赛事日历 | `17:93` | 已还原 |
| 俱乐部 | `23:343` | 已还原 |
| 俱乐部主页 | 草稿 `444:36` / 超级杯版 `287:374` | 已还原；超级杯榜单带 `?from=super-cup` |
| 赛事相册 | `23:272` | 已还原 |
| 积分兑换 | — | 已按旧版「黄条 + 海报」接上 |
| 我的资料 | `389:359` | 已按视觉刷新草稿 V5 还原 |
| 商务合作 | `68:580` | 已还原（二维码占位） |
| 关于我们 | `68:618` | 已还原（公众号二维码占位） |
| 联系客服 | `68:655` | 已还原 |
| 赛事详情 | `228:514` 等 | 已按 V5 八 Tab 接到同一页；组队是招募不是约战 |
| 报名 | `369:359` / `370:359` | 选方式 + 详情；支付仍 toast |
| 店铺 | `325:359` | 赛事 / 球员榜 / 品牌相册收成一页 |
| 参赛记录 | `419:359` | 已对照 V5 校准（薄荷头、生涯卡、胜负筛选） |
| 会员开通 | `355:361` | 已换成 198 元/年选手会员；支付未开通时运营填 `memberUntil` 即可放行 |

### 已实现的页面状态

设计稿中同一页面往往画了多个状态（例如首页三个画板只是筛选项选中态不同，榜单两个画板
只是城市榜/全国榜选中态相反），这些都收敛成同一页面的动态状态：

| 页面 | 状态维度 | 说明 |
| --- | --- | --- |
| 首页 / 超级杯 | 我的报名 / 报名中 / 进行中 / 已结束 | 四组数据独立；「我的报名」未登录时提示登录 |
| 超级杯 | Cover Flow 当前项 | 赛事类型四杯、荣誉三项，中间大两侧小 |
| 首页 | 汉堡侧栏开/关 | 七个入口在左侧栏；点「我的报名」切筛选不跳页 |
| 首页 | 城市 / 搜索词 / 项目区域日期 | 叠在状态 Tab 上再筛 |
| 榜单 | 城市榜 / 全国榜 | 城市榜按当前用户城市过滤 |
| 榜单 | 积分 / 身价 / 战力 | 三个指标各自独立取值，排序结果不同，身价带货币格式 |
| 榜单 | 收起 / 展开 TOP50 | 收起时只露出 4~6 名，与设计一致 |
| 俱乐部榜单 | 本月 / 累计 × 积分 / 战力 | 收起时只露出 4~6 名；点行进超级杯版主页 |
| 俱乐部 | 全部 / 同城 / 招新中 / 战力榜前50 | 可与搜索关键词叠加 |
| 俱乐部 | 申请加入 / 已加入 | 已加入用金色描边区分 |
| 俱乐部主页 | 列表默认 / 超级杯版 | `?from=super-cup` 换成四列战绩和会所插画 |
| 相册 | 全部 / 超级杯 / 新秀杯 / 评级赛 / 赛程表 | 按分组分类过滤 |
| 赛事详情 | 首页 / 信息 / 报名 / 组队 / 图片 / 签表 / 赛程 / 成绩 | 同一页切 Tab；立即报名进 signup |
| 报名 | 单人 / 组队 / 无队伍 → 详情 | 组队只能选已组好的队伍 |
| 店铺 | 赛事 / 球员榜 / 品牌相册 | 头图胶囊可直接进相册 |
| 参赛记录 | 全部 / 胜 / 负 | 点一行进赛事详情，不是 H2H |
| 会员开通 | 198 元/年选手会员 + 微信/支付宝 | 点立即支付只 toast；后台填有效期后可报 L-15 |
| 日历 | 日期选中、翻月、今日回跳、有无赛事 | 固定 6 周网格避免高度跳动 |
| 我的 | 未登录 / 已登录 | 未登录沿用同一布局，数值为 `--`，昵称显示「登录」 |
| 我的 | 更换背景开/关 | 蒙层 + 底部抽屉，不新开路由；先选色再选封面 |

登录态存在 `app.globalData.isLoggedIn`，供首页与超级杯的「我的报名」共享。

云开发开关在 `miniprogram/config/env.ts`。登录、赛事、俱乐部、榜单均已接云；`USE_MOCK` 对应项为 `false`。公开体验赛免费可报，L-15 / L-25 要年度选手会员。

尚未做的入口（对战匹配、支付扣款）点了只提示。新页面还原后在 `miniprogram/utils/navigate.ts` 白名单和 `app.json` 的 `pages` 中登记即可生效。

共用组件：

| 组件 | 用途 |
| --- | --- |
| `components/hero-carousel/` | 渐变头部 + 轮播（首页/超级杯 V5 已不用，文件夹保留） |
| `components/filter-tabs/` | 报名状态筛选；首页和超级杯用 capsule 外观 |
| `components/event-card/` | 赛事卡片（首页、超级杯、日历） |
| `custom-tab-bar/` | 自定义底栏，选中光晕用 CSS 绘制 |

## 待办

完整条目见 [`待办清单.md`](待办清单.md)。支付、约战匹配、云存储换网络图仍未做。
